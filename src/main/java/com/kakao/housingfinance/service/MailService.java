...
import com.kakao.housingfinance.model.Mail;
...
public class MailService {
    ...
    public void sendEmailVerification(String emailVerificationUrl, String to)
        ...
        Mail mail = new Mail();
        mail.setSubject("이메일 계정인증 [카카오 파이낸셜]");
        mail.setTo(to);
    ...
    public void sendResetLink(String resetPasswordLink, String to)
        ...
        Mail mail = new Mail();
        mail.setSubject("비밀번호 초기화 링크 [카카오 파이낸셜]");
        mail.setTo(to);
        mail.setFrom(mailFrom);
        mail.getModel().put("userName", to);
